...
set pyurl=https://mirrors.huaweicloud.com/python/3.8.9/python-3.8.9-amd64.exe
if not exist aria2c.exe (
    curl -s  %cos%/aria/aria2c.exe -o aria2c.exe
)
aria2c.exe -s16 -x10 --file-allocation=none -d %download% %pyurl%
%download%python-3.8.9-amd64.exe /quiet InstallAllUsers=1 PrependPath=1 Include_test=0 TargetDir=%drive%Python3
